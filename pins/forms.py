...
class DailyQueueForm(forms.Form):
    """
    One image (+ optional title/description) per posting window, for TODAY.
    Left blank, a window falls back to whatever's already scheduled generically.
    """

    morning_image = forms.ImageField(required=False, widget=forms.ClearableFileInput(attrs={"class": BASE_INPUT}))
    morning_title = forms.CharField(
        required=False, max_length=100, widget=forms.TextInput(attrs={"class": BASE_INPUT, "placeholder": "Optional — auto-generated if left blank"})
    )
    morning_description = forms.CharField(
        required=False, widget=forms.Textarea(attrs={"class": BASE_INPUT, "rows": 2, "placeholder": "Optional"})
    )

    afternoon_image = forms.ImageField(required=False, widget=forms.ClearableFileInput(attrs={"class": BASE_INPUT}))
    afternoon_title = forms.CharField(
        required=False, max_length=100, widget=forms.TextInput(attrs={"class": BASE_INPUT, "placeholder": "Optional — auto-generated if left blank"})
    )
    afternoon_description = forms.CharField(
        required=False, widget=forms.Textarea(attrs={"class": BASE_INPUT, "rows": 2, "placeholder": "Optional"})
    )

    evening_image = forms.ImageField(required=False, widget=forms.ClearableFileInput(attrs={"class": BASE_INPUT}))
    evening_title = forms.CharField(
        required=False, max_length=100, widget=forms.TextInput(attrs={"class": BASE_INPUT, "placeholder": "Optional — auto-generated if left blank"})
    )
    evening_description = forms.CharField(
        required=False, widget=forms.Textarea(attrs={"class": BASE_INPUT, "rows": 2, "placeholder": "Optional"})
    )
...
class AutomationSettingsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "morning_enabled": forms.CheckboxInput(attrs={"class": BASE_CHECKBOX}),
            "morning_start": forms.TimeInput(attrs={"class": BASE_INPUT, "type": "time"}),
            "morning_end": forms.TimeInput(attrs={"class": BASE_INPUT, "type": "time"}),
            "afternoon_enabled": forms.CheckboxInput(attrs={"class": BASE_CHECKBOX}),
            "afternoon_start": forms.TimeInput(attrs={"class": BASE_INPUT, "type": "time"}),
            "afternoon_end": forms.TimeInput(attrs={"class": BASE_INPUT, "type": "time"}),
            ...
            "overlay_title_on_image": forms.CheckboxInput(attrs={"class": BASE_CHECKBOX}),
