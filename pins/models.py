...
class AutomationSettings(models.Model):
    ...
    # Populated automatically by the "Connect with Pinterest" OAuth flow
    # (see pins/services/pinterest_oauth.py). pinterest_access_token above is
    ...
    overlay_title_on_image = models.BooleanField(
        default=False,
        help_text=(
            "If on, each pin's title is overlaid as text on its image right before posting "
            "(tries Gemini first if configured, always falls back to a local text overlay)."
        ),
    )
    ...
    @property
    def pinterest_is_connected(self):
        """True once the OAuth flow has stored a token for this account."""
        return bool(self.pinterest_access_token)

    @property
    def pinterest_token_is_expired(self):
        if not self.pinterest_token_expires_at:
            return False
        return timezone.now() >= self.pinterest_token_expires_at
